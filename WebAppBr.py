...
def Tester():
    st.write("""# Simple Home Credit Default Risk Prediction App using machine learning""")
    ...
    st.subheader('XGBClassifier Algorithms Results (Predictions) ')
    ...
    if df is not None:
        # load_clf = bz2.BZ2File(open('Models/XGBClassifier.pbz2', 'rb'))  # commpressed model
        # load_clf = pickle.load(load_clf)  # commpressed model
        load_clf = pickle.load(open('XGBClassifier.pkl', 'rb'))
        print('load_clf is loaded')
